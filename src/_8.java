public class _8 {
    //    https://www.projecteuler.net/problem=8
    public static void main(String[] args) {
        long _zEr0tWoZeRo0nEoNenInEnInEnInE_ThIsIsOnLyFoRBeNcHmArKiNgNoThInGElSe = System.nanoTime();
        long result = 0;
        String number = "";
        long max = Long.MIN_VALUE;
        int div = number.charAt(0) - '0';
        long temp = 1L;
        boolean zero = false;
        int N = 4;
        int zcount = N;
        for (int i = 1 ;i <= number.length(); i++) {
            if(i <=N  || zero){
                temp*= number.charAt(i - 1) - '0';
                if(--zcount == 0) zero = false;
                continue;
            }
            zero = false;
            if(number.charAt(i - N - 1) - '0' != 0) {
                temp /= number.charAt(i - N - 1) - '0';
            }
            if((number.charAt(i - 1) - '0') == 0) {temp = 1;zero = true;zcount=N;}
            else {
                temp *= number.charAt(i - 1) - '0';
            }
            if(temp > max) max = temp;
        }
        result = max;


        _zEr0tWoZeRo0nEoNenInEnInEnInE_ThIsIsOnLyFoRBeNcHmArKiNgNoThInGElSe -= System.nanoTime();
        System.out.println("result="+result);
        System.out.println("seconds="+Math.abs(_zEr0tWoZeRo0nEoNenInEnInEnInE_ThIsIsOnLyFoRBeNcHmArKiNgNoThInGElSe)/(1000.0*1000.0*1000.0));
    }
}
